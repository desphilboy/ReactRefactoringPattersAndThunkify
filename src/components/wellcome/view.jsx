...
const Wellcome = ({ handleSubmit }) => {
  const petsList = useSelector((state) => state.lookUp.nameTypes.map((nameType) => nameType.name));
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          ...
          <Field key="1" name="selectedName" component="select" width="20">
            <option key="noValue" />
            {petsList.map((pet, index) => <option key={index} value={pet}>{pet}</option>)}
          </Field>
          <br />
          or:
          <br />
        ...
        <br />
        <button style={{ margin: '50px' }} type="submit">Submit</button>
      </form>
    </div>
  );
};
